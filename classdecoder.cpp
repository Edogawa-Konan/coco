...
unsigned int bytestoint(const unsigned char *a, unsigned int *length) {
    /* 从a指向的地址中，得到一个词的编码。length指向下一个字节*/
    unsigned int result = 0;
    unsigned char b;
    unsigned int i = 0;  // byte pointer
    if (length != nullptr) *length = 0;
    do {
        b = *(a + i);
        if (b >> 7) {  //如果最高位为1，下一个byte依旧保持这个class。
            //high
            result += (b ^ 128u) * static_cast<int>(pow(128, i));
        } else {
            result += b * static_cast<int>(pow(128, i));
            if (length != nullptr) *length = i + 1;
            break;
        }
        i++;
    } while (true);
...
void
ClassDecoder::decodefile(const string &filename, std::ostream *out, unsigned int start, unsigned int end, bool quiet) {
...
std::string
ClassDecoder::decodefiletostring(const std::string &filename, unsigned int start, unsigned int end, bool quiet) {
...
unsigned char getdataversion(std::istream *in) {
    ...
    if (!in->good()) {
        cerr
                << "ERROR: Supplied data file can not be opened. Check whether it exists and whether you have proper permissions..."
                << endl;
        throw InternalError();
    }
    if (in->tellg() > 0) {
    ...
    if (b == 0xa2) {
        ...
        if (b > 5) {
            cerr
                    << "ERROR: Supplied data file is not a valid Colibri Data file, did you pass plain-text instead perhaps?..."
                    << endl;
            throw InternalError();
        }
        in->seekg(0); //reset
