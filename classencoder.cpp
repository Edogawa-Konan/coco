...
ClassEncoder::ClassEncoder(const unsigned int minlength, unsigned const int maxlength) {
    highestclass = 6; //5 and lower are reserved
...
void
ClassEncoder::processcorpus(istream *IN, unordered_map<string, unsigned int> &freqlist, unordered_set<string> *vocab) {
    while (IN->good()) {
        string line;
        getline(*IN, line);
        int begin = 0;
        ...
        for (int i = 0; i < s; i++) {
            if ((line[i] == ' ') || (i == s - 1)) {
                ...
                if ((word.length() > 0) && (word != "\r") && (word != "\t") && (word != " ")) {
                    ...
                    if ((vocab == nullptr) || ((vocab != nullptr) && (vocab->find(word) != vocab->end()))) {
                        if ((minlength > 0) || (maxlength > 0)) {
                            const unsigned int l = (unsigned int) utf8_strlen(word);
...
void ClassEncoder::buildclasses(const unordered_map<string, unsigned int> &freqlist, unsigned int threshold) {
    ...
    for (unordered_map<string, unsigned int>::const_iterator iter = freqlist.begin(); iter != freqlist.end(); iter++) {
        if (iter->second >= threshold)
            revfreqlist.insert(pair<const unsigned int, const string>(-1 * iter->second, iter->first));
    }

    auto cls = highestclass;
    for (multimap<const unsigned int, const string>::const_iterator iter = revfreqlist.begin();
         iter != revfreqlist.end(); iter++) {
        if (!classes.count(
                iter->second)) { //check if it doesn't already exist, in case we are expanding on existing classes
            cls++;
...
void ClassEncoder::build(vector<string> &files, bool quiet, unsigned int threshold, const string vocabfile) {
    ...
    for (vector<string>::iterator iter = files.begin(); iter != files.end(); iter++) {
        ...
        processcorpus(filename, freqlist, &vocab);
    }
    buildclasses(freqlist, threshold);
}
...
void ClassEncoder::save(const string &filename) {
    ...
    for (std::unordered_map<std::string, unsigned int>::iterator iter = classes.begin();
    ...
    for (int i = 0; i < l; i++) {
        if ((line[i] == ' ') || (i == l - 1)) {
            ...
            if ((word.length() > 0) && (word != "\r") && (word != "\t") && (word != " ")) {
                ...
                classlength = inttobytes(nullptr, cls);
...
unsigned int ClassEncoder::encodestring(const string &line, unsigned char *outputbuffer, bool allowunknown, bool autoaddunknown,
                               unsigned int *nroftokens) {
    unsigned int outputcursor = 0;
    int begin = 0;
    const int l = line.length();
    unsigned int classlength;
    for (int i = 0; i < l; i++) {
        if ((line[i] == ' ') || (i == l - 1)) {
            ...
            if ((word.length() > 0) && (word != "\r") && (word != "\t") && (word != " ")) {
                ...
                if (word == "{*}") {
                    ...
                    if (nroftokens != nullptr) (*nroftokens)++;
                    continue;
                } else if (word == "{**}") {
                ...
                classlength = inttobytes(outputbuffer + outputcursor, cls);  //占用的byte数
                outputcursor += classlength;
                if (nroftokens != nullptr) (*nroftokens)++;
            }
        }
...
Pattern
ClassEncoder::buildpattern(const std::string &patternstring, bool allowunknown, bool autoaddunknown) { //not thread-safe
...
Pattern ClassEncoder::buildpattern_safe(const std::string &patternstring, bool allowunknown,
                                        bool autoaddunknown) { //thread-safe
...
void ClassEncoder::encodefile(const std::string &inputfilename, const std::string &outputfilename, bool allowunknown,
                              bool autoaddunknown, bool append, bool ignorenewlines, bool quiet) {
    ofstream OUT;
    ifstream IN;
    if (append) {
...
void
ClassEncoder::encodefile(istream *IN, ostream *OUT, bool allowunknown, bool autoaddunknown, bool quiet, bool append,
                         bool ignorenewlines) {
    if (!append) {
        //写入版本号信息162+2
    ...
    const char zero = 0;  // 就是delimiterclass
    ...
    unsigned int nroftokens; //非token的字符
    while (IN->good()) {
        nroftokens = 0;
        string line;
        getline(*IN, line);
        ...
        linenum++;
        if (line.length() >
            outputbuffersize) { //heuristic to check if we need to compute the length, string will be longer than encoded representation
        ...
        outputsize = encodestring(line, outputbuffer, allowunknown, autoaddunknown, &nroftokens);  // 编码该句子
        if (ignorenewlines) {
            if (totalnroftokens + nroftokens >= 65536) { //max token count (uint16)
                if (totalnroftokens == 0) {
                    cerr
                            << "ERROR: Each input line may not contain more than 65536 tokens/words. Limit exceeded on line "
                            << linenum << " (" << nroftokens << " tokens)" << endl;
                    throw InternalError();
                }
    ...
    if (!quiet) cerr << "ClassEncoder encoded " << linenum << " lines" << endl;
    delete[] outputbuffer;
}

void ClassEncoder::buildNer(const NerCorpus &nerCorpus) {
    std::cerr<<"before encoding Ner, highest cls is "<<this->gethighestclass()<<"\n";
    unsigned int token_count = 0;
    for(auto & nv: nerCorpus.data)
    {
        for (auto & ner: nv)
        {
            if(this->find(ner.raw_string) == this->end())
            {
                this->add(ner.raw_string, this->gethighestclass()+1);
                token_count++;
            }
        }
    }
    std::cerr<<"ClassEncoder encoded Ner "<<token_count<<" , now highest cls is "<<this->highestclass<<"\n";
}

PatternPointer
ClassEncoder::buildpatternpointer(const std::string &patternstring, bool allowunknown, bool autoaddunknown) {
    int buffersize = encodestring(patternstring, buildbuffer, allowunknown, autoaddunknown);
    if (buffersize > buildbuffersize) {
        cerr << "INTERNAL ERROR: Exceeded buildpattern buffer size" << endl;
        exit(2);
    }
    PatternPointer pattern = PatternPointer(buildbuffer, buffersize);
    return pattern;
}
...
unsigned char *convert_v1_v2(const unsigned char *olddata, unsigned int &newlength) {
    ...
    do {
        ...
        if (c == 0) {
            ...
            newlength += inttobytes(nullptr, cls);
            i += c + 1;
...
unsigned char *convert_v1_v2(istream *in, bool ignoreeol, bool debug) {
    ...
    do {
        ...
        if (readingdata) {
            ...
            if (c == 0) {
                ...
                //we have a size
                if (c == 0) {
                    std::cerr << "ERROR: Pattern length is zero according to input stream.. not possible! (stage 1)"
                              << std::endl;
                    throw InternalError();
                } else {
                    readingdata = c;
    ...
    if ((beginposcheck != beginpos)
        ...
        std::cerr << "ERROR: After resetting readpointer for stage 2, istream is not 'good': eof=" << (int) in->eof()
    ...
    while (i < length) {
        if (in->good()) {
            ...
            std::cerr << "ERROR: Invalid pattern data, unexpected end of file (stage 2,i=" << i << ",length=" << length
                      << ",beginpos=" << beginpos << ",eof=" << (int) in->eof() << ",fail=" << (int) in->fail()
        ...
        if (readingdata) {
            ...
            if (c == 0) {
                ...
                //we have a size
                if (c == 0) {
                    std::cerr << "ERROR: Pattern length is zero according to input stream.. not possible! (stage 2)"
                              << std::endl;
                    delete[] data;
                    throw InternalError();
                } else {
