...
public:
    ...
    static const unsigned char nerclass = 5;
     ...
     */
    ClassEncoder(unsigned int minlength = 0, unsigned int maxlength = 0);
     ...
     * @param maxlength Maximum supported length of words (default: 0 = unlimited)
     */
    ClassEncoder(const std::string &filename, unsigned int minlength = 0,
                          unsigned int maxlength = 0); //load an existing classer
     ...
     * @param maxlength Maximum supported length of words (default: 0 = unlimited)
     */
    void load(const std::string &filename, unsigned int minlength = 0,
              unsigned int maxlength = 0); //load an existing classer
     ...
     */
    void build(const std::string &filename, unsigned int threshold = 0, std::string vocabfile = "");

    /**
     * Encoding from Ner information
     * @param nerCorpus NerCorpus object, represent all ner in corpus
     */
    void buildNer(const NerCorpus& nerCorpus);
     ...
     */
    void
    build(std::vector<std::string> &files, bool quiet = false, unsigned int threshold = 0, std::string vocabfile = "");
    ...
    void processcorpus(const std::string &filename, std::unordered_map<std::string, unsigned int> &freqlist,
                       std::unordered_set<std::string> *vocab = nullptr);
     ...
     */
    unsigned int
    encodestring(const std::string &line, unsigned char *outputbuffer, bool allowunknown, bool autoaddunknown = false,
                 unsigned int *nroftokens = nullptr);
     ...
     * @return The number of bytes written to outputbuffer
     */
    void encodefile(const std::string &inputfilename, const std::string &outputfilename, bool allowunknown,
                    bool autoaddunknown = false, bool append = false, bool ignorenewlines = false, bool quiet = false);
    ...
    void encodefile(std::istream *IN, std::ostream *OUT, bool allowunknown, bool autoaddunknown, bool quiet = false,
                    bool append = false, bool ignorenewlines = false);
     ...
     * @return a Pattern
     */
    Pattern buildpattern(const std::string &patternstring, bool allowunknown = false,
                         bool autoaddunknown = false);  //not thread-safe


    /**
     * Build a patternpointer from a string.
     * **Note:** This function is not thread-safe! Use buildpattern_safe() instead if you need thread safety!
     * @param patternstring The string you want to turn into a Pattern
     * @param allowunknown If the string contains unknown words, represent those using a single unknown class. If set to false, an exception will be raised when unknown words are present. (default: false)
     * @param autoaddunknown If the string contains unknown words, automatically add these words to the class encoding. Note that the class encoding will no longer be optimal if this is used. (default: false)
     * @return a PatternPointer
     */
    PatternPointer buildpatternpointer(const std::string &patternstring, bool allowunknown = false,
                                        bool autoaddunknown = false);  //not thread-safe
    /**
     ...
     * @return a Pattern
     */
    Pattern buildpattern_safe(const std::string &patternstring, bool allowunknown = false,
                              bool autoaddunknown = false);  //thread-safe


    /**
     * Add the word with the specified class to the class encoding
     */
    void add(const std::string &, unsigned int cls);
    ...
    /**
     * test whether contains specific key
     * @param key word
     * @return true if contain else false
     */
    bool contains(const std::string & key) { return this->find(key)!= this->end(); }
...
int countwords(const unsigned char *data, int l);
