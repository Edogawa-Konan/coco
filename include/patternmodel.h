...
public:
    ...
    /**
         ...
         * @param constrainmodel Load only patterns that occur in this model
         */
    PatternSetModel(const std::string &filename, const PatternModelOptions &options,
    ...
    /**
         ...
         */
    virtual void
    load(std::string &filename, const PatternModelOptions &options, PatternModelInterface *constrainmodel = NULL) {
    ...
    /**
         ...
         * @param constrainmodel Load only patterns that occur in this model
         */
    virtual void load(std::istream *f, const PatternModelOptions &options,
                      PatternModelInterface *constrainmodel = NULL) { //load from file
        char null;
        ...
        if ((null != 0) || ((model_type != UNINDEXEDPATTERNMODEL) && (model_type != INDEXEDPATTERNMODEL) &&
                            (model_type != PATTERNSETMODEL) && (model_type != PATTERNALIGNMENTMODEL))) {
            std::cerr << "ERROR: File is not a colibri patternmodel file" << std::endl;
            throw InternalError();
        }
        if (model_version > 2) {
            std::cerr
                    << "WARNING: Model is created with a newer version of Colibri Core! Attempting to continue but failure is likely..."
                    << std::endl;
        }
        ...
        if (options.DEBUG) {
            std::cerr << "Debug enabled, loading PatternModel type " << (int) model_type << ", version "
                      << (int) model_version << ", classencodingversion" << (int) this->classencodingversion
                      << std::endl;
            std::cerr << "Total tokens: " << totaltokens << ", total types: " << totaltypes << std::endl;;
        }
        if (model_type == PATTERNSETMODEL) {
            //reading set
            PatternSet<uint64_t>::read(f, options.MINLENGTH, options.MAXLENGTH, constrainstore, !options.DOREMOVENGRAMS,
                                       !options.DOREMOVESKIPGRAMS, !options.DOREMOVEFLEXGRAMS); //read PatternStore
        } else if (model_type == INDEXEDPATTERNMODEL) {
            //reading from indexed pattern model, ok:
            readmap<IndexedData, IndexedDataHandler>(f, options.MINTOKENS, options.MINLENGTH, options.MAXLENGTH,
                                                     constrainstore, !options.DOREMOVENGRAMS,
                                                     !options.DOREMOVESKIPGRAMS, !options.DOREMOVEFLEXGRAMS);
        } else if (model_type == UNINDEXEDPATTERNMODEL) {
            //reading from unindexed pattern model, ok:
            readmap<uint32_t, BaseValueHandler<uint32_t>>(f, options.MINTOKENS, options.MINLENGTH, options.MAXLENGTH,
                                                          constrainstore, !options.DOREMOVENGRAMS,
                                                          !options.DOREMOVESKIPGRAMS, !options.DOREMOVEFLEXGRAMS);
        } else if (model_type == PATTERNALIGNMENTMODEL) {
            //ok:
            readmap<PatternFeatureVectorMap<double>, PatternFeatureVectorMapHandler<double>>(f, options.MINTOKENS,
                                                                                             options.MINLENGTH,
                                                                                             options.MAXLENGTH,
                                                                                             constrainstore,
                                                                                             !options.DOREMOVENGRAMS,
                                                                                             !options.DOREMOVESKIPGRAMS,
                                                                                             !options.DOREMOVEFLEXGRAMS);
        } else {
...
//    ClassDecoder *classDecoder; // decode entire corpus
    ...
    /**
         ...
         */
    PatternModel<ValueType, ValueHandler, MapType, PatternType>(std::istream *f, PatternModelOptions options,
                                                                PatternModelInterface *constrainmodel = NULL,
    ...
    /**
         ...
         * @param corpus Pointer to the loaded corpus, used as a reverse index.
         */
    PatternModel<ValueType, ValueHandler, MapType, PatternType>(const std::string &filename,
                                                                const PatternModelOptions &options,
                                                                PatternModelInterface *constrainmodel = NULL,
                                                                IndexedCorpus *corpus = NULL) { //load from file
    ...
    virtual void load(std::istream *f, const PatternModelOptions &options,
        ...
        f->read((char *) &model_version, sizeof(char));
        if (model_version == 1) this->classencodingversion = 1;
        if ((null != 0) || ((model_type != UNINDEXEDPATTERNMODEL) && (model_type != UNINDEXEDPATTERNPOINTERMODEL) &&
                            (model_type != INDEXEDPATTERNMODEL) && (model_type != INDEXEDPATTERNPOINTERMODEL) &&
                            (model_type != PATTERNALIGNMENTMODEL))) {
        ...
        if (((model_type == INDEXEDPATTERNMODEL) && (this->getmodeltype() == UNINDEXEDPATTERNMODEL)) ||
            ((model_type == INDEXEDPATTERNPOINTERMODEL) && (this->getmodeltype() == UNINDEXEDPATTERNPOINTERMODEL))) {
            //reading indexed pattern model as unindexed, (or indexed patternPOINTErmodels as unindexed patternPOINTERmodels)
            MapType::template read<IndexedData, IndexedDataHandler, PatternType>(f, options.MINTOKENS,
            ...
            MapType::template read<uint32_t, BaseValueHandler<uint32_t>, PatternType>(f, options.MINTOKENS,
                                                                                      options.MINLENGTH,
                                                                                      options.MAXLENGTH, constrainstore,
                                                                                      !options.DOREMOVENGRAMS,
                                                                                      !options.DOREMOVESKIPGRAMS,
                                                                                      !options.DOREMOVEFLEXGRAMS,
                                                                                      options.DORESET, options.DEBUG);
            ...
            MapType::template read<uint32_t, BaseValueHandler<uint32_t>, PatternPointer>(f, options.MINTOKENS,
                                                                                         options.MINLENGTH,
                                                                                         options.MAXLENGTH,
                                                                                         constrainstore,
                                                                                         !options.DOREMOVENGRAMS,
                                                                                         ...
                                                                                         options.DORESET,
                                                                                         options.DEBUG);
        } else if ((model_type == INDEXEDPATTERNPOINTERMODEL) &&
                   ((this->getmodeltype() == INDEXEDPATTERNMODEL) || (this->getmodeltype() == UNINDEXEDPATTERNMODEL))) {
            //reading indexed patternpointermodel as (un)indexed patternmodel
            MapType::template read<IndexedData, IndexedDataHandler, PatternPointer>(f, options.MINTOKENS,
                                                                                    options.MINLENGTH,
                                                                                    options.MAXLENGTH, constrainstore,
                                                                                    !options.DOREMOVENGRAMS,
                                                                                    !options.DOREMOVESKIPGRAMS,
            ...
            MapType::template read<PatternFeatureVectorMap<double>, PatternFeatureVectorMapHandler<double>, PatternType>(
                    f, options.MINTOKENS, options.MINLENGTH, options.MAXLENGTH, constrainstore, !options.DOREMOVENGRAMS,
                    !options.DOREMOVESKIPGRAMS, !options.DOREMOVEFLEXGRAMS, options.DORESET, options.DEBUG);
        } else {
            MapType::template read(f, options.MINTOKENS, options.MINLENGTH, options.MAXLENGTH, constrainstore,
                                   !options.DOREMOVENGRAMS, !options.DOREMOVESKIPGRAMS, !options.DOREMOVEFLEXGRAMS,
                                   options.DORESET,
    ...
    /** Train a pattern model on corpus data with ner attribute
     *
     *
     * */
    virtual void train_with_ner(const std::string &filename,
    ...
    /**
         ...
         */
    virtual void train(std::istream *in, PatternModelOptions options, PatternModelInterface *constrainbymodel = nullptr,
                       PatternSet<> *filter = nullptr, bool continued = false, uint32_t firstsentence = 1,
                       bool ignoreerrors = false, ClassDecoder *classDecoder = nullptr) {
        ...
        if (constrainbymodel == this) {
            totaltypes = 0;
            totaltokens = 0;
        } else if (constrainbymodel != nullptr) {
        ...
        if (!options.QUIET) {
            ...
            if (constrainbymodel != nullptr) std::cerr << ", constrained by another model";
        ...
        if (constrainbymodel != nullptr) {
            if ((options.DOSKIPGRAMS) && (!options.DOSKIPGRAMS_EXHAUSTIVE)) {
                if (constrainbymodel != this) {
                    ...
                    if (!options.QUIET)
                        std::cerr
                                << "WARNING: Skipgrams will be extracted exhaustively on the basis of the ngrams found; the constraint model will be applied only afterwards. This implies some skipgrams in the constraint model that are present may be missed, and it will not be most efficient. Use in-place rebuilding of your constraint model instead."
                                << std::endl;
                    if (options.MAXLENGTH >= 31)
                        std::cerr
                                << "WARNING: No maximum pattern length set or maximum pattern length is very high! This will drastically slow down skipgram computation"
                                << std::endl;
                }
            }
        }
        ...
        if (!this->data.empty()) {
            if ((continued) && (!options.QUIET))
                std::cerr << "Continuing training on preloaded model, computing statistics..." << std::endl;
            this->computestats();
        }
        ...
        for (int n = 1; n <= options.MAXLENGTH; n++) {
            ...
            if (continued) {
                if ((options.MINTOKENS > 1) && (constrainbymodel == nullptr)) {
                    if (cache_grouptotal[NGRAM][n] > 0) {
                        if ((options.DOSKIPGRAMS_EXHAUSTIVE) && (cache_grouptotal[SKIPGRAM][n] == 0)) {
                            skipgramsonly = true;
                        } else {
                            if (!options.QUIET)
                                std::cerr << "Skipping " << n << "-grams, already in model" << std::endl;
                            continue;
                        }
                    }
                }
            ...
            if ((options.DOSKIPGRAMS_EXHAUSTIVE) && (gapmasks[n].empty()))
                gapmasks[n] = compute_skip_configurations(n, options.MAXSKIPS);
            ...
            while (((reverseindex != NULL) && (sentence < sentences)) ||
                ...
                sentence++;
                //read line
                if (linepattern != nullptr) delete linepattern;
                ...
                if (options.DOPATTERNPERLINE) {
                    ...
                    if (iter_unigramsonly) {
                    ...
                    } else if ((options.MINTOKENS > 1) && (constrainbymodel == nullptr)) {
                        line.ngrams(ngrams, n); //
                    } else {
                        singlepass = true;
                        ...
                        line.subngrams(ngrams, minlength,
                                       options.MAXLENGTH); //extract ALL ngrams if MINTOKENS == 1 or a constraint model is set, no need to look back anyway, only one iteration over corpus
                    }
                }
                ...
                for (std::vector<std::pair<PatternPointer, int>>::iterator iter = ngrams.begin();
                    ...
                    try {
                        ...
                        if (!skipgramsonly) {
                            ...
                            if ((constrainbymodel != nullptr) && (!iter_unigramsonly) &&
                                (!constrainbymodel->has(iter->first)))
                                continue;  //skip when we have a constraintmodel and the pattern is not found
                            ...
                            if ((found) && (filter != nullptr) && (constrainbymodel == nullptr)) {
                                //special behaviour: filter
                                ignorefilter = false;
                                if (((options.MINTOKENS > 1) && (n >= options.MINLENGTH)) ||
                                    ((options.MINTOKENS == 1) &&
                                     (iter->first.n() >= (unsigned int) options.MINLENGTH))) {
                                    ...
                                    if (filterhasngrams) {
                                        subngrams.clear();
                                        iter->first.subngrams(subngrams, 1,
                                                              options.MINTOKENS > 1 ? n : iter->first.n());
                                        for (std::vector<PatternPointer>::iterator iter2 = subngrams.begin();
                                             iter2 != subngrams.end(); iter2++) {
                                            if (filter->has(*iter2)) {
                                                matchfilter = true;
                                    ...
                                    if ((!matchfilter) && (filterhasskipgrams)) {
                                        for (PatternSet<Pattern>::iterator iter2 = filter->begin();
                                             iter2 != filter->end(); iter2++) {
                            ...
                            if ((found) && (!skipgramsonly)) {
                                if (options.DEBUG)
                                    std::cerr << "\t\tAdding @" << ref.sentence << ":" << ref.token << " n="
                                              ...
                                              << "   " << iter->first.tostring(*classDecoder) << std::endl;
                                add(iter->first, ref);
                            } else {
                                if (options.DEBUG) {
                        ...
                        if (((n >= 3) || (options.MINTOKENS == 1)) //n is always 1 when mintokens == 1
                            ...
                            ref = IndexReference(sentence,
                                                 iter->second); //this is one token, we add the tokens as we find them, one by one
                            int foundskipgrams_thisround = this->computeskipgrams(iter->first, options, &ref, NULL,
                                                                                  constrainbymodel, true);
            ...
            if (!iter_unigramsonly) {
                ...
                if ((!continued) && ((constrainbymodel == nullptr) or (constrainbymodel == this))) {
                ...
                if ((!continued) && ((constrainbymodel == nullptr) || (constrainbymodel == this))) {
                    if (!options.QUIET) std::cerr << " computing total word types prior to pruning...";
            ...
            for (int n = begin_n; n > 1; n--) {
                ...
                while (iter != this->end()) {
                    ...
                    if (pattern_n == (unsigned int) n) {
                        ...
                        for (std::vector<PatternPointer>::iterator iter2 = subngrams.begin();
                             iter2 != subngrams.end(); iter2++)
                            subsumed.insert(Pattern(*iter2));
                    }
                    iter++;
                };
                prunednonsubsumed += this->prunenotinset(subsumed, n - 1);
                if (!options.QUIET)
                    std::cerr << " pruned " << prunednonsubsumed << " non-subsumed " << (n - 1) << "-grams"
                              << std::endl;
            }
        }
    ...
    /**
         ...
         */
    virtual void
    train(const std::string &filename, PatternModelOptions options, PatternModelInterface *constrainbymodel = nullptr,
          PatternSet<> *filter = nullptr, bool continued = false, uint32_t firstsentence = 1,
          bool ignoreerrors = false, ClassDecoder *classDecoder = nullptr) {
        std::ifstream *in = new std::ifstream(filename.c_str());
    ...
    /**
         ...
         */
    virtual int
    computeskipgrams(const PatternPointer &pattern, int mintokens = 2, const IndexReference *singleref = nullptr,
                     const IndexedData *multiplerefs = nullptr, PatternModelInterface *constrainbymodel = nullptr,
                     std::vector<PatternPointer> *targetcontainer = nullptr, const bool exhaustive = false,
                     const int maxskips = 3, const bool DEBUG = false) {
        ...
        int gapconf_i = 0;
        for (std::vector<uint32_t>::iterator iter2 = gapmasks[n].begin();
             iter2 != gapmasks[n].end(); iter2++, gapconf_i++) {
            ...
            try {
                ...
                if ((constrainbymodel != nullptr) && (!constrainbymodel->has(skipgram))) continue;
...
//                    }
                    ...
                    if (check_extra) {
                        if (exhaustive) { //the following is by definition the case in non-exhaustive mode, so we need only do it in exhaustive mode:
                            ...
                            for (std::vector<PatternPointer>::iterator iter3 = parts.begin();
                                 iter3 != parts.end(); iter3++) {
                ...
                if (skipgram_valid) {
                    ...
                    if (targetcontainer == nullptr) {
                        ...
                        if (singleref != nullptr) {
                            ...
                            std::cerr
                                    << "ERROR: computeskipgrams() called with no singleref, no multiplerefs and no targetcontainer"
                                    << std::endl;
                            throw InternalError();
                        }
                    } else {
    ...
    /**
         ...
         */
    virtual int computeskipgrams(const PatternPointer &pattern, PatternModelOptions &options,
                                 const IndexReference *singleref = NULL, const IndexedData *multiplerefs = NULL,
                                 PatternModelInterface *constrainbymodel = NULL,
                                 const bool exhaustive = false) { //backward compatibility
        if (options.MINTOKENS_SKIPGRAMS < options.MINTOKENS) options.MINTOKENS_SKIPGRAMS = options.MINTOKENS;
        return computeskipgrams(pattern, options.MINTOKENS_SKIPGRAMS, singleref, multiplerefs, constrainbymodel, NULL,
                                exhaustive, options.MAXSKIPS, options.DEBUG);
    }

    virtual int computeskipgrams_with_ner(const PatternPointer &pattern, PatternModelOptions &options,
    ...
    /**
        ...
        * given pattern
         */
    virtual std::vector<PatternPointer>
    findskipgrams(const PatternPointer &pattern, unsigned int occurrencethreshold = 1, int maxskips = 3) {
        //given the pattern, find all skipgrams in it that occur in the model

        std::vector<PatternPointer> skipgrams;
        this->computeskipgrams(pattern, occurrencethreshold, NULL, NULL, this->getinterface(), &skipgrams, false,
                               maxskips);
        return skipgrams;
    }
    ...
    /**
         ...
         */
    virtual void trainskipgrams(const PatternModelOptions options, PatternModelInterface *constrainbymodel = nullptr) {
        if (constrainbymodel == this) {
            trainskipgrams_selfconstrained(options);
        } else {
            std::cerr << "Can not compute skipgrams on unindexed model (except exhaustively during train() )"
                      << std::endl;
            throw InternalError();
        }
    }

    virtual void trainskipgrams_with_ner(PatternModelOptions options) {
        std::cerr << "Can not compute skipgrams on unindexed model (except exhaustively during train() )"
                  << std::endl;
    }
    ...
    virtual void trainskipgrams_selfconstrained(PatternModelOptions options) {
        ...
        if (!options.QUIET)
            std::cerr << "Finding skipgrams/flexgrams matching preloaded constraints, occurrence threshold "
                      << options.MINTOKENS << " ..." << std::endl;
        ...
        for (IndexedCorpus::iterator iter = this->reverseindex->begin(); iter != this->reverseindex->end(); iter++) {
            ...
            std::unordered_set<PatternPointer> skipgrams = this->getreverseindex(ref, 0,
                                                                                 SKIPGRAMORFLEXGRAM);  //will also match flexgrams
            if (options.DEBUG && !skipgrams.empty())
                std::cerr << skipgrams.size() << " skipgrams found @" << ref << std::endl;
            for (std::unordered_set<PatternPointer>::iterator iter = skipgrams.begin();
                 iter != skipgrams.end(); iter++) {
        ...
        if ((!foundskipgrams) && (!foundflexgrams)) {
            ...
            if (!options.QUIET)
                std::cerr << " Found " << foundskipgrams << " skipgrams, " << foundflexgrams << " flexgrams ...";
        }
    ...
    void write(std::ostream *out) {
        ...
        unsigned char v = this->getmodelversion();
        out->write((char *) &v, sizeof(char));
        if ((this->getmodeltype() == UNINDEXEDPATTERNPOINTERMODEL) ||
            (this->getmodeltype() == INDEXEDPATTERNPOINTERMODEL)) {
    ...
    /**
         ...
         */
    std::unordered_set<PatternPointer>
    getreverseindex(const IndexReference ref, int occurrencecount = 0, int category = 0, unsigned int size = 0,
                    int = 3) {
        ...
        for (unsigned int n = minn; ref.token + n <= sl && n <= maxn; n++) {
            if ((size == 0) || (n == size)) {
                try {
                    ...
                    if ((((occurrencecount == 0) && this->has(ngram)) ||
                         ((occurrencecount != 0) && (this->occurrencecount(ngram) >= (unsigned int) occurrencecount)))
                    ...
                    if ((includeskipgrams) && (n >= 3)) {
                        ...
                        if (iter != matchskipgramhelper.end()) {
                            for (std::vector<std::pair<uint32_t, unsigned char>>::iterator iter2 = iter->second.begin();
                                 iter2 != iter->second.end(); iter2++) {
                                if (n == iter2->second) {
                                    PatternPointer skipgram = ngram;
                                    skipgram.mask = iter2->first;
                                    if (((occurrencecount == 0) && this->has(skipgram)) || ((occurrencecount != 0) &&
        ...
        if (includeflexgrams) {
            try {
                ...
                if (iter != this->matchflexgramhelper.end()) {
                    for (std::unordered_set<PatternPointer>::iterator iter2 = iter->second.begin();
                         iter2 != iter->second.end(); iter2++) {
                        PatternPointer flexgram = *iter2;
                        try {
                            flexgram = this->reverseindex->getflexgram(ref, flexgram);
                            if (((occurrencecount == 0) && this->has(flexgram)) || ((occurrencecount != 0) &&
                                                                                    (this->occurrencecount(flexgram) >=
                                                                                     (unsigned int) occurrencecount))) {
    ...
    /**
         ...
         * @param size Set to any value above zero to only include patterns of the specified length.
         */
    std::vector<std::pair<IndexReference, PatternPointer>>
    getreverseindex_bysentence(int sentence, int occurrencecount = 0, int category = 0, unsigned int size = 0) {
        //Auxiliary function
        ...
        for (unsigned int i = 0; i < this->reverseindex->sentencelength(sentence); i++) {
            ...
            for (std::unordered_set<PatternPointer>::iterator iter = tmpresult.begin();
                 iter != tmpresult.end(); iter++) {
    ...
    /**
         ...
         */
    std::vector<std::pair<IndexReference, PatternPointer>>
    getreverseindex_right(const IndexReference ref, int occurrencecount = 0, int category = 0, unsigned int size = 0) {
        //Auxiliary function
        std::vector<std::pair<IndexReference, PatternPointer>> result;
        for (unsigned int i = ref.token + 1; i < this->reverseindex->sentencelength(ref.sentence); i++) {
            ...
            for (std::unordered_set<PatternPointer>::iterator iter = tmpresult.begin();
                 iter != tmpresult.end(); iter++) {
                const PatternPointer pattern = *iter;
                result.emplace_back(ref2, pattern);
            }
        }
        return result;
    }
    ...
    /**
         ...
         */
    std::vector<std::pair<IndexReference, PatternPointer>>
    getreverseindex_left(const IndexReference ref, int occurrencecount = 0, int category = 0, unsigned int size = 0) {
        //Auxiliary function
        std::vector<std::pair<IndexReference, PatternPointer>> result;
...
//            }
                    ...
                    if (((n == 0) || (*itern == n)) && (!cache_processed[*iterc][*itern])) {
                        ...
                        while (iter != this->end()) {
                            ...
                            if ((pn == 1) && (*itern <= 1) && ((*iterc == 0) || (pattern.category() == *iterc))) {
                                ...
                                if (((*itern == 0) || (pn == *itern)) &&
                                    ((*iterc == 0) || (pattern.category() == *iterc))) {
                                    std::vector<PatternType> unigrams;
                                    pattern.ngrams(unigrams, 1);
                                    for (typename std::vector<PatternType>::iterator iter2 = unigrams.begin();
                                         iter2 != unigrams.end(); iter2++) {
    ...
    unsigned int totaloccurrencesingroup(int category, int n) {
        ...
        if ((cache_grouptotal.empty()) && (!this->data.empty()))
            this->computestats();
    ...
    virtual void add(const PatternPointer &patternpointer, const IndexReference &ref) {
        ...
        this->add(pattern, data, ref);   // ValueType is IndexData, put indexReference into IndexData
    }
...
//                if(!iter->first.ners.empty())
//                    std::cerr<< "prune ner " << iter->first.ners[0].raw_string<<std::endl;
    ...
    virtual void print(std::ostream *out, ClassDecoder &decoder, bool instantiate = false) {
        ...
        if (haveoutput) {
            ...
            std::cerr
                    ...
                    << std::endl;
            std::cerr << " - COUNT      : The occurrence count - the amount of times the pattern occurs in the data"
                      << std::endl;
            std::cerr
        ...
        for (IndexedCorpus::iterator iter = reverseindex->begin(); iter != reverseindex->end(); iter++) {
            ...
            *out << ref.tostring();
            for (std::unordered_set<PatternPointer>::iterator iter2 = rindex.begin(); iter2 != rindex.end(); iter2++) {
                const Pattern p = (const Pattern &) *iter2;
    ...
    void printmodel(std::ostream *out,
                    ClassDecoder &decoder) { //an alias because cython can't deal with a method named print
    ...
    /**
         ...
         */
    virtual void
    print(std::ostream *out, ClassDecoder &decoder, const PatternType &pattern, bool= false, bool endline = true) {
        ...
        *out << pattern_s << "\t" << count << "\t" << "\t" << covcount << "\t" << coverage << "\t" << cat_s << "\t"
             << pattern.size() << "\t" << freq;
    ...
    /**
         ...
         */
    void printpattern(std::ostream *out, ClassDecoder &decoder, const Pattern &pattern, bool instantiate = false,
                      bool endline = true) {  //another alias for cython who can't deal with methods named print
    ...
    /**
         ...
         */
    void histogram(std::map<unsigned int, unsigned int> &hist, unsigned int threshold = 0, unsigned int cap = 0,
                   int category = 0, unsigned int size = 0) {
        for (PatternModel::iterator iter = this->begin(); iter != this->end(); iter++) {
            const PatternType pattern = iter->first;
            if (((category != 0) && (pattern.category() != category)) ||
                ((size != 0) && (size != pattern.size())))
                continue;
    ...
    /**
         ...
         * @param size Set to any value above zero to only include only patterns of the specified length. (0 for all sizes)
         */
    void histogram(std::ostream *OUT, unsigned int threshold = 0, unsigned int cap = 0, int category = 0,
                   unsigned int size = 0) {
        std::map<unsigned int, unsigned int> hist;
    ...
    void info(std::ostream *OUT) {
        ...
        *OUT << "Total key bytesize (patterns): " << totalkeybs << " bytes (" << (totalkeybs / 1024 / 1024) << " MB)"
             << std::endl;
        *OUT << "Total value bytesize (counts/index): " << totalvaluebs << " bytes (" << (totalvaluebs / 1024 / 1024)
             << " MB)" << std::endl;
        ...
        *OUT << "Mean value bytesize: " << (totalvaluebs / (float) this->size()) << std::endl;
        ...
        if (this->reverseindex) {
            for (IndexedCorpus::iterator iter = this->reverseindex->begin();
                 iter != this->reverseindex->end(); iter++) {
            ...
            *OUT << "Total key bytesize in reverse index (references): " << ri_totalkeybs << " bytes ("
                 << (ri_totalkeybs / 1024 / 1024) << " MB)" << std::endl;
            *OUT << "Total value bytesize in reverse index (patterns): " << ri_totalvaluebs << " bytes ("
                 << (ri_totalvaluebs / 1024 / 1024) << " MB)" << std::endl;
        }
    ...
    void report(std::ostream *OUT, bool nocoverage = false) {
        ...
        *OUT << "REPORT" << std::endl;
        if ((this->getmodeltype() == UNINDEXEDPATTERNMODEL) && (!nocoverage)) {
            ...
            *OUT << "            assuming no overlap at all!!! Use an indexed model for accurate coverage counts"
                 << std::endl;
        }
        *OUT << "----------------------------------" << std::endl;
        *OUT << "                          " << std::setw(15) << "PATTERNS" << std::setw(15) << "TOKENS"
             << std::setw(15) << "COVERAGE" << std::setw(15) << "TYPES" << std::setw(15) << std::endl;
        *OUT << "Total:                    " << std::setw(15) << "-" << std::setw(15) << this->tokens() << std::setw(15)
             << "-" << std::setw(15) << this->types() << std::endl;
        ...
        if (!nocoverage) {
            ...
            *OUT << "Uncovered:                " << std::setw(15) << "-" << std::setw(15) << uncoveredtokens
                 << std::setw(15) << uncoveredtokens / (double) this->tokens() << std::setw(15)
                 << this->types() - coveredtypes << std::endl;
            *OUT << "Covered:                  " << std::setw(15) << this->size() << std::setw(15) << coveredtokens
                 << std::setw(15) << coveredtokens / (double) this->tokens() << std::setw(15) << coveredtypes
                 << std::endl << std::endl;
        } else {
            *OUT << std::endl;
        }
            ...
            if (cache_grouptotalpatterns.count(c))
                for (std::set<int>::iterator itern = cache_n.begin(); itern != cache_n.end(); itern++) {
                    ...
                    if (cache_grouptotalpatterns[c].count(n)) {
                        if (!haveoutput) {
                            //output headers
                            *OUT << std::setw(15) << "CATEGORY" << std::setw(15) << "N (SIZE) " << std::setw(15)
                                 << "PATTERNS";
                            if ((this->getmodeltype() != UNINDEXEDPATTERNMODEL) && (!nocoverage))
                                *OUT << std::setw(15) << "TOKENS" << std::setw(15) << "COVERAGE";
        ...
        if (haveoutput) {
            ...
            if (this->getmodeltype() != UNINDEXEDPATTERNMODEL) {
                std::cerr << " - TOKENS      : The number of tokens that is covered by the patterns in the group."
                          << std::endl;
                std::cerr << " - COVERAGE    : The number of tokens covered, as a fraction of the total in the corpus"
                          << std::endl;
            }
    ...
    virtual void outputrelations(const PatternPointer &, ClassDecoder &, std::ostream *, const std::string = "",
                                 bool= true) {} //does nothing for unindexed models
    virtual t_relationmap getsubchildren(const PatternPointer &, int = 0, int = 0,
                                         int = 0) { return t_relationmap(); } //does nothing for unindexed models
    virtual t_relationmap getsubparents(const PatternPointer &, int = 0, int = 0,
                                        int = 0) { return t_relationmap(); } //does nothing for unindexed models
    virtual t_relationmap
    gettemplates(const PatternPointer &, int = 0) { return t_relationmap(); } //does nothing for unindexed models
    virtual t_relationmap
    getinstances(const PatternPointer &, int = 0) { return t_relationmap(); } //does nothing for unindexed models
    virtual t_relationmap
    getskipcontent(const PatternPointer &) { return t_relationmap(); } //does nothing for unindexed models
    virtual t_relationmap getleftneighbours(const PatternPointer &, int = 0, int = 0, int = 0,
                                            int = 0) { return t_relationmap(); } //does nothing for unindexed models
    virtual t_relationmap getrightneighbours(const PatternPointer &, int = 0, int = 0, int = 0,
                                             int = 0) { return t_relationmap(); } //does nothing for unindexed models
    virtual t_relationmap_double
    getnpmi(const Pattern &, double) { return t_relationmap_double(); } //does nothing for unindexed models

    virtual int computeflexgrams_fromskipgrams() { return 0; } //does nothing for unindexed models

    virtual int computeflexgrams_fromskipgrams_withners(const PatternModelOptions &options,
                                                        const ClassDecoder *classdecoder) { return 0; }
...
void PatternModel<ValueType, ValueHandler, MapType, PatternType>::train_with_ner(const std::string &filename,
                                                                                 PatternModelOptions options,
                                                                                 NerCorpus &nerCorpus,
                                                                                 ClassEncoder *classEncoder,
                                                                                 ClassDecoder *classDecoder,
...
//                        }
    ...
    if (options.DOSKIPGRAMS && !options.DOSKIPGRAMS_EXHAUSTIVE) {
        this->trainskipgrams_with_ner(options);
    }
    if (options.MINTOKENS == 1) {
        //needed to compute maxn, minn
        this->postread(options);
...
protected:
    virtual void postread(const PatternModelOptions &) override {
        for (typename PatternModel<IndexedData, IndexedDataHandler, MapType>::iterator iter = this->begin();
             iter != this->end(); iter++) {
            const Pattern p = iter->first;
            const PatternCategory category = p.category();
            const int n = p.n();
    ...
    virtual void posttrain(const PatternModelOptions &options) override {
...
public:
    ...
    * Begin a new pattern model, optionally pre-setting a reverseindex.
    */
    IndexedPatternModel<MapType, PatternType>(IndexedCorpus *corpus = nullptr)
            : PatternModel<IndexedData, IndexedDataHandler, MapType, PatternType>() {
...
//            this->classDecoder = classDecoder;
    ...
    * @param corpus Pointer to the loaded corpus, used as a reverse index.
    */
    IndexedPatternModel<MapType, PatternType>(std::istream *f, const PatternModelOptions options,
                                              PatternModelInterface *constrainmodel = NULL,
                                              IndexedCorpus *corpus = NULL)
            : PatternModel<IndexedData, IndexedDataHandler, MapType, PatternType>() { //load from file
        this->model_type = this->getmodeltype();
        this->model_version = this->getmodelversion();
        if (corpus) {
            this->reverseindex = corpus;
    ...
    * @param corpus Pointer to the loaded corpus, used as a reverse index.
    */
    IndexedPatternModel<MapType, PatternType>(const std::string filename, const PatternModelOptions options,
                                              PatternModelInterface *constrainmodel = NULL,
                                              IndexedCorpus *corpus = NULL)
            : PatternModel<IndexedData, IndexedDataHandler, MapType, PatternType>() { //load from file
        this->model_type = this->getmodeltype();
     ...
     */
    virtual void add(const Pattern &pattern, IndexedData *value, const IndexReference &ref) override {
        if (value == nullptr) {
    ...
    virtual void add(const PatternPointer &patternpointer, IndexedData *value, const IndexReference &ref) override {
        if (value == NULL) {
    ...
    virtual void train_with_ner(const std::string &filename, PatternModelOptions options, NerCorpus &nerCorpus,
                                ClassEncoder *classEncoder, ClassDecoder *classDecoder = nullptr,
                                uint32_t firstsentence = 1,
                                bool ignoreerrors = false) override {
        if ((options.DOSKIPGRAMS) && (this->reverseindex == nullptr)) {
            std::cerr
                    << "ERROR: You must specify a reverse index if you want to train skipgrams (or train skipgrams exhaustively)"
                    << std::endl;
            throw InternalError();
        }
        PatternModel<IndexedData, IndexedDataHandler, MapType, PatternType>::train_with_ner(filename, options,
                                                                                            nerCorpus, classEncoder,
                                                                                            classDecoder,
                                                                                            firstsentence,
                                                                                            ignoreerrors);
    }
    ...
    virtual void train(std::istream *in, PatternModelOptions options, PatternModelInterface *constrainbymodel = nullptr,
                       PatternSet<> *filter = nullptr, bool continued = false, uint32_t firstsentence = 1,
                       bool ignoreerrors = false, ClassDecoder *classDecoder = nullptr) override {
        if ((options.DOSKIPGRAMS) && (this->reverseindex == nullptr)) {
            std::cerr
                    << "ERROR: You must specify a reverse index if you want to train skipgrams (or train skipgrams exhaustively)"
                    << std::endl;
        ...
        PatternModel<IndexedData, IndexedDataHandler, MapType, PatternType>::train(in, options, constrainbymodel,
                                                                                   filter, continued, firstsentence,
                                                                                   ignoreerrors, classDecoder);
    }
    ...
    train(const std::string &filename, PatternModelOptions options, PatternModelInterface *constrainbymodel = nullptr,
        ...
        PatternModel<IndexedData, IndexedDataHandler, MapType, PatternType>::train(filename, options, constrainbymodel,
                                                                                   filter, continued, firstsentence,
                                                                                   ignoreerrors, classDecoder);
    }
    ...
    */
    void print(std::ostream *out, ClassDecoder &decoder, bool instantiate = false) override {
        ...
        for (typename PatternModel<IndexedData, IndexedDataHandler, MapType, PatternType>::iterator iter = this->begin();
             iter != this->end(); iter++) {
...
//                *out << "PATTERN\tCOUNT\tTOKENS\tCOVERAGE\tCATEGORY\tSIZE\tFREQUENCY\tREFERENCES" << std::endl;
    ...
    void print(std::ostream *out, ClassDecoder &decoder, const Pattern &pattern, bool instantiate = false,
               bool endline = true) override {
...
//        const double coverage = covcount / (double) this->tokens();
//        const double freq = this->frequency(pattern);
        const double coverage = 0;
        const double freq = 0;
        const size_t covcount = 0;
...
//             << pattern.size() << "\t" << freq << "\t";
        ...
        *out << pattern_s << "\t" << count << "\t" << cat_s << "\t";
        ...
        for (IndexedData::iterator iter2 = data->begin(); iter2 != data->end(); iter2++) {
            ...
            *out << iter2->tostring();
            if (cat != NGRAM) {
                if ((this->reverseindex != nullptr) && (instantiate)) {
...
//        out<<buffer.rdbuf();
    ...
    virtual void trainskipgrams_with_ner(PatternModelOptions options) override {
        if (options.MINTOKENS == -1) options.MINTOKENS = 2;
        this->cache_grouptotal.clear(); //forces recomputation of statistics
        ...
        for (int n = 3; n <= options.MAXLENGTH; n++) {
            ...
            for (typename MapType::iterator iter = this->begin(); iter != this->end(); iter++) {
                ...
                if ((pattern.n() == n) && (pattern.category() == NGRAM))
                    foundskipgrams += this->computeskipgrams_with_ner(pattern, options, nullptr, &multirefs);
            }
            if (!foundskipgrams) {
                std::cerr << " None found" << std::endl;
                break;
     ...
     */
    virtual void
    trainskipgrams(PatternModelOptions options, PatternModelInterface *constrainbymodel = nullptr) override {
        if (options.MINTOKENS == -1) options.MINTOKENS = 2;
        this->cache_grouptotal.clear(); //forces recomputation of statistics
        if (constrainbymodel == this) {
            ...
            for (int n = 3; n <= options.MAXLENGTH; n++) {
                ...
                for (typename MapType::iterator iter = this->begin(); iter != this->end(); iter++) {
                    ...
                    const IndexedData multirefs = iter->second;
                    if ((pattern.n() == n) && (pattern.category() == NGRAM))
                        foundskipgrams += this->computeskipgrams(pattern, options, NULL, &multirefs, constrainbymodel,
                                                                 false);
                }
                if (!foundskipgrams) {
                    std::cerr << " None found" << std::endl;
    ...
    t_relationmap getskipcontent(const PatternPointer &pattern) override {
        ...
        if (pattern.category() != NGRAM) {
            ...
            for (IndexedData::const_iterator iter2 = data->begin(); iter2 != data->end(); iter2++) {
                ...
                Pattern skipcontent_atref = PatternPointer(skipcontent_atref_raw, head,
                                                           n - head - tail); //pattern from patternpointer
        ...
        for (IndexedData::iterator iter = data->begin(); iter != data->end(); iter++) {
            ...
            for (std::unordered_set<PatternPointer>::iterator iter2 = rindex.begin(); iter2 != rindex.end(); iter2++) {
                ...
                if (((int) candidate.n() == _n) && (candidate != pattern) && (candidate.category() == SKIPGRAM) &&
                    ((occurrencethreshold == 0) || (this->occurrencecount(pattern) >= occurrencethreshold))) {
    ...
    * @return a relation map
    */
    t_relationmap getsubchildren(const PatternPointer &pattern, unsigned int occurrencethreshold = 0, int category = 0,
                                 unsigned int size = 0) {
        if ((this->reverseindex == NULL) || (this->reverseindex->empty())) {
            std::cerr << "ERROR: No reverse index present" << std::endl;
            throw InternalError();
        ...
        for (IndexedData::iterator iter = data->begin(); iter != data->end(); iter++) {
            ...
            for (int i = ref.token; i < ref.token + _n; i++) {
                ...
                //search in reverse index
                std::unordered_set<PatternPointer> rindex = this->getreverseindex(begin, occurrencethreshold, category,
                                                                                  size);
                for (std::unordered_set<PatternPointer>::iterator iter2 = rindex.begin();
                     iter2 != rindex.end(); iter2++) {
                    const PatternPointer candidate = *iter2;
    ...
    * @return a relation map
    */
    t_relationmap getsubparents(const PatternPointer &pattern, unsigned int occurrencethreshold = 0, int category = 0,
                                unsigned int size = 0) {
        ...
        for (IndexedData::iterator iter = data->begin(); iter != data->end(); iter++) {
            ...
            //search in reverse index
            std::vector<std::pair<IndexReference, PatternPointer>> rindex = this->getreverseindex_bysentence(
                    ref.sentence, occurrencethreshold, category, size);
            for (std::vector<std::pair<IndexReference, PatternPointer>>::iterator iter2 = rindex.begin();
                 iter2 != rindex.end(); iter2++) {
                ...
                if (((int) candidate.n() >= minsubsize) && (candidate != pattern)
                    ...
                    if ((candidate.category() == SKIPGRAM) || (pattern.category() == SKIPGRAM)) {
                        ...
                        Pattern inst = Pattern(candidate, iter2->first.token,
                                               pattern.n()); //get the proper slice to match
    ...
    */
    t_relationmap
    getleftneighbours(const PatternPointer &pattern, unsigned int occurrencethreshold = 0, int category = 0,
                      unsigned int size = 0, unsigned int cutoff = 0) {
        if ((this->reverseindex == NULL) || (this->reverseindex->empty())) {
            std::cerr << "ERROR: No reverse index present" << std::endl;
            throw InternalError();
        }
    ...
    */
    t_relationmap
    getrightneighbours(const PatternPointer &pattern, unsigned int occurrencethreshold = 0, int category = 0,
                       unsigned int size = 0, unsigned int cutoff = 0) {
        if ((this->reverseindex == NULL) || (this->reverseindex->empty())) {
            std::cerr << "ERROR: No reverse index present" << std::endl;
            throw InternalError();
    ...
    * after computation.
    */
    virtual void computecoveragestats(int category = 0, int n = 0) override {
        //opting for memory over speed (more iterations, less memory)
        ...
        for (std::set<int>::iterator iterc = this->cache_categories.begin();
             iterc != this->cache_categories.end(); iterc++) {
            if ((category == 0) || (*iterc == category)) {
                        ...
                        while (iter != this->end()) {
                            ...
                            if ((n == 1) && (*itern <= 1) && ((*iterc == 0) || (pattern.category() == *iterc))) {
                                ...
                                if (((*itern == 0) || (n == *itern)) &&
                                    ...
                                    pattern.ngrams(unigrams, 1);
                                    for (std::vector<PatternPointer>::iterator iter2 = unigrams.begin();
                                         iter2 != unigrams.end(); iter2++) {
                            ...
                            if (((*itern == 0) || (n == *itern)) && ((*iterc == 0) || (pattern.category() == *iterc))) {
                                IndexedData *data = this->getdata(pattern);
                                for (IndexedData::iterator dataiter = data->begin();
    ...
    * @return a relation map
    */
    t_relationmap getrightcooc(const PatternPointer &pattern, unsigned int occurrencethreshold = 0, int category = 0,
                               unsigned int size = 0,
                               std::vector<std::pair<IndexReference, IndexReference>> *matches = nullptr) {
        if ((this->reverseindex == NULL) || (this->reverseindex->empty())) {
            std::cerr << "ERROR: No reverse index present" << std::endl;
            throw InternalError();
        }
        ...
        for (IndexedData::iterator iter = data->begin(); iter != data->end(); iter++) {
            ...
            for (std::vector<std::pair<IndexReference, PatternPointer>>::iterator iter2 = rindex.begin();
                ...
                if ((ref2.token > ref.token + _n)
                    ...
                    cooc[neighbour]++;
                    if (matches != nullptr) matches->push_back(std::pair<IndexReference, IndexReference>(ref, ref2));
                }
            }
        }
    ...
    * @return a relation map
    */
    t_relationmap getleftcooc(const PatternPointer &pattern, unsigned int occurrencethreshold = 0, int category = 0,
                              unsigned int size = 0) {
        if ((this->reverseindex == NULL) || (this->reverseindex->empty())) {
            std::cerr << "ERROR: No reverse index present" << std::endl;
            throw InternalError();
    ...
    * @return a relation map
    */
    t_relationmap getcooc(const PatternPointer &pattern, unsigned int occurrencethreshold = 0, int category = 0,
                          unsigned int size = 0, bool ordersignificant = false) {
        if ((this->reverseindex == NULL) || (this->reverseindex->empty())) {
            std::cerr << "ERROR: No reverse index present" << std::endl;
            throw InternalError();
        }
        ...
        for (IndexedData::iterator iter = data->begin(); iter != data->end(); iter++) {
            ...
            for (std::vector<std::pair<IndexReference, PatternPointer>>::iterator iter2 = rindex.begin();
                ...
                const IndexReference ref2 = iter2->first;
                const PatternPointer neighbour = iter2->second;
                if ((ordersignificant) && (neighbour.pattern() < static_cast<const Pattern>(pattern))) continue;
    ...
    double npmi(const PatternPointer &key1, const PatternPointer &key2, int jointcount) {
        ...
        return log((double) jointcount / (this->occurrencecount(key1) * this->occurrencecount(key2))) /
               -log((double) jointcount / (double) this->totaloccurrencesingroup(0, 0));
    }
     ...
     */
    void outputrelations(const PatternPointer &pattern, t_relationmap &relations, ClassDecoder &classdecoder,
                         std::ostream *OUT, const std::string label = "RELATED-TO") {
        int total = 0;
        ...
        for (t_relationmap::iterator iter = relations.begin(); iter != relations.end(); iter++) {
            ...
            *OUT << "\t" << pattern_s << "\t" << label << "\t" << pattern2.tostring(classdecoder) << "\t"
                 << iter->second << "\t" << iter->second / total_f << "\t" << this->occurrencecount(pattern2)
                 << std::endl;
        }
    }
     ...
     */
    void outputrelations(const PatternPointer &pattern, ClassDecoder &classdecoder, std::ostream *OUT,
                         const std::string filter = "", bool outputheader = true) override {
     ...
     */
    void computenpmi(std::map<PatternPointer, t_relationmap_double> &coocmap, double threshold, bool right = true,
                     bool left = true) {
     ...
     */
    void
    computecooc(std::map<PatternPointer, t_relationmap> &coocmap, int threshold, bool right = true, bool left = true) {
        for (typename PatternModel<IndexedData, IndexedDataHandler, MapType, PatternType>::iterator iter = this->begin();
             iter != this->end(); iter++) {
            const PatternType pattern = iter->first;
            t_relationmap tmp;
    ...
    int computeflexgrams_fromskipgrams_withners(const PatternModelOptions &options,
                                                const ClassDecoder *classdecoder) override {
        this->cache_grouptotal.clear(); //forces recomputation of statistics
        int count = 0;
        for (typename PatternModel<IndexedData, IndexedDataHandler, MapType, PatternType>::iterator iter = this->begin();
             iter != this->end(); iter++) {
            PatternType pattern = iter->first;
            if (pattern.category() == SKIPGRAM) {
                if (options.DEBUG && classdecoder != nullptr) {
                    std::cerr << pattern.tostring(*classdecoder) << "    ->    ";
                }
                const PatternType flexgram = pattern.toflexgram_with_ner();
                if (!this->has(flexgram)) count++;
                //copy data from pattern
                IndexedData *data = this->getdata(pattern);
//                if(data== &this->data[flexgram])
//                    std::cerr<< "error!";
                for (auto ref : *data) {
                    this->data[flexgram].insert(ref);
                }
                if (options.DEBUG && classdecoder != nullptr) {
                    std::cerr << flexgram.tostring(*classdecoder) << " " << data->size() << "="
                              << this->data[flexgram].size() << "   ||   ";
                    for (const Ner &ner: flexgram.ners)
                        std::cerr << "  " << ner;
                    std::cerr << std::endl;
                }
            }
        ...
        for (typename PatternModel<IndexedData, IndexedDataHandler, MapType, PatternType>::iterator iter = this->begin();
            ...
            for (t_relationmap::iterator iter2 = tmp.begin(); iter2 != tmp.end(); iter2++) {
                ...
                if (value >= threshold) {
                    ...
                    for (std::vector<std::pair<IndexReference, IndexReference>>::iterator iter3 = matches.begin();
                         iter3 != matches.end(); iter3++) {
     ...
     * @param threshold Normalised Pointwise Mutual Information threshold
     */
    void outputcooc_npmi(std::ostream *OUT, ClassDecoder &classdecoder, double threshold) override {
        ...
        while (iter != npmimap.end()) {
            for (t_relationmap_double::iterator iter2 = iter->second.begin(); iter2 != iter->second.end(); iter2++) {
                inversemap.insert(std::pair<double, std::pair<PatternPointer, PatternPointer>>(iter2->second,
                                                                                               std::pair<Pattern, Pattern>(
                                                                                                       iter->first,
                                                                                                       iter2->first)));
            }
        ...
        *OUT << "Pattern1\tPattern2\tNPMI" << std::endl;
        for (std::multimap<double, std::pair<PatternPointer, PatternPointer>>::reverse_iterator iter2 = inversemap.rbegin();
             iter2 != inversemap.rend(); iter2++) {
            const PatternPointer pattern1 = iter2->second.first;
            const PatternPointer pattern2 = iter2->second.second;
            *OUT << pattern1.tostring(classdecoder) << "\t" << pattern2.tostring(classdecoder) << "\t" << iter2->first
                 << std::endl;
        }
    }
     ...
     * @param threshold Normalised Pointwise Mutual Information threshold
     */
    void outputcooc(std::ostream *OUT, ClassDecoder &classdecoder, double threshold) override {
        ...
        while (iter != coocmap.end()) {
            for (t_relationmap::iterator iter2 = iter->second.begin(); iter2 != iter->second.end(); iter2++) {
                inversemap.insert(std::pair<uint32_t, std::pair<PatternPointer, PatternPointer>>(iter2->second,
                                                                                                 std::pair<PatternPointer, PatternPointer>(
                                                                                                         iter->first,
                                                                                                         iter2->first)));
            }
        ...
        *OUT << "Pattern1\tPattern2\tCooc" << std::endl;
        for (std::multimap<uint32_t, std::pair<PatternPointer, PatternPointer>>::reverse_iterator iter2 = inversemap.rbegin();
             iter2 != inversemap.rend(); iter2++) {
    ...
    int flexgramsize(const Pattern &pattern, IndexReference begin) {
        ...
        int i = 0;
        std::vector<std::pair<IndexReference, PatternPointer>> rindex = this->getreverseindex_right(
                begin); //TODO: Check
        for (std::vector<std::pair<IndexReference, PatternPointer>>::iterator iter = rindex.begin();
             iter != rindex.end(); iter++) {
        ...
        for (int j = 0; j < numberofparts; j++) {
            ...
            int prevlevel = -1;
            bool found = false;
            for (std::multimap<int, IndexReference>::iterator iter = partmatches.lower_bound(j);
                 iter != partmatches.upper_bound(j); iter++) {
                found = true;
                if (iter->first != prevlevel) {
                    begin = nextbegin;
                    nextbegin = IndexReference(begin.sentence, 999); //reset
                }
                if (((iter->second == begin) || (begin < iter->second)) &&
                    (iter->second + parts[j].n() + 1 < nextbegin)) {
...
public:
    ...
    PatternPointerModel<ValueType, ValueHandler, MapType>(IndexedCorpus *corpus)
            : PatternModel<ValueType, ValueHandler, MapType, PatternPointer>() {
        this->model_type = this->getmodeltype();
        this->model_version = this->getmodelversion();
        if (corpus) {
            this->reverseindex = corpus;
    ...
    /**
        ...
        * @param corpus Pointer to the loaded corpus, used as a reverse index.
        */
    PatternPointerModel<ValueType, ValueHandler, MapType>(std::istream *f, const PatternModelOptions options,
                                                          PatternModelInterface *constrainmodel = NULL,
                                                          IndexedCorpus *corpus = NULL)
            : PatternModel<ValueType, ValueHandler, MapType, PatternPointer>() { //load from file
        this->model_type = this->getmodeltype();
        this->model_version = this->getmodelversion();
        if (corpus) {
            this->reverseindex = corpus;
    ...
    /**
        ...
        * @param corpus Pointer to the loaded corpus, used as a reverse index.
        */
    PatternPointerModel<ValueType, ValueHandler, MapType>(const std::string filename, const PatternModelOptions options,
                                                          PatternModelInterface *constrainmodel = NULL,
                                                          IndexedCorpus *corpus = NULL)
            : PatternModel<ValueType, ValueHandler, MapType, PatternPointer>() { //load from file
        this->model_type = this->getmodeltype();
    ...
    /**
         ...
         */
    virtual void add(const PatternPointer &patternpointer, const IndexReference &ref) {
        if ((patternpointer.data < this->reverseindex->beginpointer()) ||
            (patternpointer.data > this->reverseindex->beginpointer() + this->reverseindex->bytesize())) {
...
public:
    ...
    /**
        ...
        * @param corpus Pointer to the loaded corpus, used as a reverse index.
        */
    IndexedPatternPointerModel<MapType>(std::istream *f, const PatternModelOptions options,
                                        PatternModelInterface *constrainmodel = NULL, IndexedCorpus *corpus = NULL)
            : IndexedPatternModel<MapType, PatternPointer>() { //load from file
        this->model_type = this->getmodeltype();
        this->model_version = this->getmodelversion();
        if (corpus) {
            this->reverseindex = corpus;
    ...
    /**
        ...
        * @param corpus Pointer to the loaded corpus, used as a reverse index.
        */
    IndexedPatternPointerModel<MapType>(const std::string filename, const PatternModelOptions options,
                                        PatternModelInterface *constrainmodel = NULL, IndexedCorpus *corpus = NULL)
            : IndexedPatternModel<MapType, PatternPointer>() { //load from file
        this->model_type = this->getmodeltype();
        this->model_version = this->getmodelversion();
...
void comparemodels_loglikelihood(std::vector<PatternModel<uint32_t> *> &models, PatternMap<double> *resultmap,
                                 bool conjunctiononly = false, std::ostream *output = NULL,
