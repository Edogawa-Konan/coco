...
PatternCategory datacategory(const unsigned char *data, int maxbytes = 0) {
    ...
    if (data == nullptr) return category;
    int i = 0;
    unsigned int length;
    do {
...
//    if(this->bow.size() != 0)
//    {
//        if(category == SKIPGRAM)
//            category = SKIPBOW;
//        else if(category == NGRAM)
//            category = BOW;
//    }
    return category;
}
...
PatternCategory PatternPointer::category() const {
    PatternCategory category;
    if (mask == 0) {
        category = datacategory(data, bytes);
    } else if (mask > B32 / 2) {
        category =  FLEXGRAM;
    } else {
        category = SKIPGRAM;
...
bool PatternPointer::isgap(unsigned int index) const { //is the word at this position a gap?
...
Pattern Pattern::toflexgram() const { //converts a fixed skipgram into a dynamic one, ngrams just come out unchanged
    ...
    auto pattern = Pattern(mainpatternbuffer, j - 1);
//    pattern.ners = this->ners;  // copy ners from this pattern
    return pattern;
}

std::vector<bool> Pattern::ner_mask() const {
    std::vector<bool> mask(this->n(), false);
    if (this->ners.empty())
        return mask;
    auto it = this->ners.cbegin();
    for (int i = 0; i < this->n(); i++) {
        if (i >= (it->start + it->len))
            it++;
        if (it == this->ners.cend())
            break;
        if (i >= it->start && i < (it->start + it->len))
            mask[i] = true;
    }
    return mask;
}

Pattern Pattern::toflexgram_with_ner() {
    if (this->ners.empty()) { return this->toflexgram(); }

    unsigned int token_index = 0;
    auto mask = this->ner_mask();
    //to be computed in bytes
    if (data == nullptr) return *this;
    int i = 0;  //pattern data index
    int j = 0;  //buffer index
...
//            int a = 1;
        if ((!prevhigh) && (c == ClassDecoder::delimiterclass)) {
            ...
            if (mask[token_index])
                mainpatternbuffer[j++] = ClassDecoder::flexclass;
            else {
                if (!skipgap) {
                    /*将连续的skipclass统一成一个flexclass*/
                    mainpatternbuffer[j++] = ClassDecoder::flexclass; //store a DYNAMIC GAP instead
            ...
            skipgap = false;
            gap_count = 0;
            if (!prevhigh && gap_count > 1) {
                for (auto &new_ner : new_ners) {
                    if (new_ner.start >= token_index)
                        new_ner.start = new_ner.start - (gap_count - 1);
                }
            }
        }
        prevhigh = (c >= 128);
    ...
    //copy from mainpatternbuffer
    auto pattern = Pattern(mainpatternbuffer, j - 1);
    pattern.ners = new_ners;
    return pattern;
}


PatternPointer
PatternPointer::toflexgram() const { //converts a fixed skipgram into a flexgram, ngrams just come out unchanged
    PatternPointer copy = *this;
    if (mask != 0) copy.mask = mask | (1 << 31);
    return copy;
}

PatternPointer PatternPointer::toflexgram_with_ner() const {
    PatternPointer copy = *this;
    if (mask != 0) copy.mask = mask | (1 << 31);
    return copy;
}
...
//    return hasher(this->tostring(*Pattern::classDecoder));
//    auto vec = this->tovector_with_ner();
//    std::hash<int> hasher;
//    size_t seed = 0;
//    for (size_t i : vec) {
...
std::string Pattern::tostring(const ClassDecoder &classdecoder) const {
    string result = datatostring(data, classdecoder);
    if (this->ners.empty())
        return result;
    else {
        int cur = 0;
        int num = 0;  // token index
        auto it = this->ners.cbegin();
        unsigned int skip = 0;
        bool first = true;
        string with_ner_result;
        for (int i = 0; i < result.length(); i++) {
            if (result[i] == ' ' || i == result.length() - 1)  // whitespace
            {
                std::string word;
                if (result[i] == ' ')
                    word = std::string(result.begin() + cur, result.begin() + i);
                else
                    word = std::string(result.begin() + cur, result.begin() + i + 1);
                cur = i + 1;
                if (skip == 0) {
                    if (it != this->ners.cend() && it->start == num) {
                        skip = it->len - 1;  // skip the next len - 1 tokens
                        if (first) {
                            with_ner_result.append(it->raw_string);
                            first = false;
                        } else
                            with_ner_result.append(" " + it->raw_string);
                        it++;
        ...
        /* below code is too slow,  sad */
//        static istringstream ist;
//        ist.str(result);
//        ist.clear();
//        int num = 0;
//        string token;
//        string with_ner_result;
//        auto it = this->ners.cbegin();
//        bool first = true; // indicate whether token is first
//        while (ist >> token) {
//            if (it != this->ners.cend() && it->start == num) {
//                for (int count = 0; count < it->len - 1; count++)
//                {
//                    ist >> token;  //discard token
//                    num++;
//                }
//                if (first) {
//                    with_ner_result.append(it->raw_string);
...
//            num++;
//        }
        return with_ner_result;
    }
}

std::string Pattern::torawstring(const ClassDecoder &classdecoder) const
{
    string result = datatostring(data, classdecoder);
    return result;
}

std::string PatternPointer::tostringraw(const ClassDecoder &classdecoder) const
{
    std::string result;
    unsigned int i = 0;
    unsigned int n = 0;
...
std::string PatternPointer::tostring(const ClassDecoder &classdecoder) const {
    std::string result = this->tostringraw(classdecoder);
    if (this->ners.empty())
        return result;
    else {
        int cur = 0;
...
/*print the unsigned char *data by means of print cls*/
...
Pattern::Pattern(std::istream *in, bool ignoreeol, const unsigned char version, const unsigned char *, bool debug) {
    if (version == 2) {
        ...
        do {
            if (in->good()) {
                ...
                if (ignoreeol) {
                    ...
                    std::cerr << "WARNING: Unexpected end of file (stage 1, length=" << length
                              << "), no EOS marker found (adding and continuing)" << std::endl;
        ...
        if ((beginposcheck != beginpos)
            ...
            std::cerr << "ERROR: Resetting read pointer for stage 2 failed! (" << (unsigned long) beginposcheck
                      << " != " << (unsigned long) beginpos << ")" << std::endl;
            throw InternalError();
        } else if (!in->good()) {
            std::cerr << "ERROR: After resetting readpointer for stage 2, istream is not 'good': eof="
                      << (int) in->eof() << ", fail=" << (int) in->fail() << ", badbit=" << (int) in->bad()
                      << std::endl;
            throw InternalError();
        }
        while (i < length) { //TODO: read multiple bytes in one go
            if (in->good()) {
                ...
                std::cerr << "ERROR: Invalid pattern data, unexpected end of file (stage 2,i=" << i << ",length="
                          << length << ",beginpos=" << beginpos << ",eof=" << (int) in->eof() << ",fail="
                          << (int) in->fail() << ",badbit=" << (int) in->bad() << ")" << std::endl;
                throw InternalError();
            }
            data[i++] = c;
...
PatternPointer::PatternPointer(std::istream *in, bool, const unsigned char, unsigned char *corpusstart, bool debug) {
    if (corpusstart == NULL) {
        ...
        if (debug)
            std::cerr << "DEBUG read patternpointer @corpusoffset=" << (size_t) data << " bytes=" << (int) bytes
...
ClassDecoder* Pattern::classDecoder = nullptr;
...
Pattern::Pattern(const Pattern &ref, size_t begin, size_t length, size_t *byteoffset,
                 bool byteoffset_shiftbyone) { //slice constructor
    //to be computed in bytes
...
PatternPointer::PatternPointer(unsigned char *ref, size_t begin, size_t length, size_t *byteoffset,
                               bool byteoffset_shiftbyone) { //slice constructor
    //to be computed in bytes
    size_t begin_b = (byteoffset != NULL) ? *byteoffset : 0;
    size_t length_b = 0;
...
PatternPointer::PatternPointer(const Pattern &ref, size_t begin, size_t length, size_t *byteoffset,
                               bool byteoffset_shiftbyone) { //slice constructor
    //to be computed in bytes
    size_t begin_b = (byteoffset != NULL) ? *byteoffset : 0;
    size_t length_b = 0;
...
PatternPointer::PatternPointer(const PatternPointer &ref, size_t begin, size_t length, size_t *byteoffset,
                               bool byteoffset_shiftbyone, bool enable_ner) { //slice constructor
    //to be computed in bytes
    size_t begin_b = (byteoffset != nullptr) ? *byteoffset : 0;
    size_t length_b = 0;
    bool prevhigh = false;

    size_t i = (byteoffset != nullptr) ? *byteoffset : 0;
    size_t n = 0;
    unsigned char c;
    do {
        if (i == ref.bytes) {
        ...
        if (c < 128) {
            //we have a token
            n++;
            if ((n == 1) && (byteoffset_shiftbyone) && (byteoffset != nullptr)) *byteoffset = i + 1;
            if (n - begin == length) {
                length_b = (i + 1) - begin_b;
                break;
            } else if ((c == ClassDecoder::delimiterclass) && (!prevhigh)) {
    ...
    if ((byteoffset != nullptr) && (!byteoffset_shiftbyone)) *byteoffset = i + 1;
    data = ref.data + begin_b;
    bytes = length_b;
    mask = computemask();

    /* add below code to process pattern with ner entity */
    if (enable_ner && !ref.ners.empty()) {
        auto it = std::lower_bound(ref.ners.begin(), ref.ners.end(), begin_b,
                                   [](const Ner &obj, const unsigned int &start) { return obj.start < start; });

        for (; it != ref.ners.cend() && it->start >= begin_b && (it->start + it->len) <= (begin_b + n); it++) {
            Ner ner = *it;
            ner.start -= begin_b;
            this->ners.push_back(ner);
        }
    }
...
Pattern::Pattern(const Pattern &ref) { //copy constructor
    if ((ref.data != nullptr) && (ref.data[0] != 0)) {
...
Pattern::Pattern(const PatternPointer &ref, size_t begin, size_t length, size_t *byteoffset,
                 bool byteoffset_shiftbyone) { //slice constructor from patternpointer
    //to be computed in bytes
    size_t begin_b = (byteoffset != NULL) ? *byteoffset : 0;
    size_t length_b = 0;
    bool prevhigh = false;
...
bool Pattern::equals(const Pattern &other) const {
    if ((data == nullptr) || (data[0] == 0)) {
        return (other.data == nullptr || other.data[0] == 0);
    } else if ((other.data == nullptr) || (other.data[0] == 0)) {
        return false;
    }
    size_t i = 0;
    do {
...
bool PatternPointer::equals(const Pattern &other) const {
    if ((data == nullptr) || (data[0] == 0)) {
        return (other.data == nullptr || other.data[0] == 0);
    } else if ((other.data == nullptr) || (other.data[0] == 0)) {
        return false;
...
bool PatternPointer::equals(const PatternPointer &other) const {
    if (bytes == other.bytes) {
        if ((mask != 0) && (isflexgram())) {  // compare flexgram
            ...
            /* mask equal, either ngram or skipgram*/
            size_t i = 0;
            size_t n = 0;
            while (i < bytes) {
                if ((mask != 0) && (data[i] < 128)) {
...
bool PatternPointer::operator==(const PatternPointer &other) const {  //todo
    return this->equals(other);
}
...
int Pattern::ngrams(vector<PatternPointer> &container,
                    const int n) const { //return multiple ngrams, also includes skipgrams!
    const int _n = this->n();
    if (n > _n) return 0;
    int found = 0;
    size_t byteoffset = 0;
...
int PatternPointer::ngrams(vector<PatternPointer> &container,
                           const int n) const { //return multiple ngrams, also includes skipgrams!
    const int _n = this->n();
    if (n > _n) return 0;
    int found = 0;
...
int PatternPointer::bow_ngrams(std::vector<PatternPointer> &container, int n) const {
    const int _n = this->n();
    if (n > _n) return 0;
    int found = 0;
    size_t byteoffset = 0;
    for (int i = 0; i < (_n - n) + 1; i++) {
        container.push_back(PatternPointer(*this, 0, n, &byteoffset, true, true));
        container.back().compute_bow(n);
        found++;
    }
    return found;
}

int Pattern::ngrams(vector<pair<Pattern, int>> &container,
                    const int n) const { //return multiple ngrams, also includes skipgrams!
    const int _n = this->n();
    if (n > _n) return 0;
    int found = 0;
...
int Pattern::ngrams(vector<pair<PatternPointer, int>> &container,
                    const int n) const { //return multiple ngrams, also includes skipgrams!
    const int _n = this->n();
    if (n > _n) return 0;
...
int PatternPointer::ngrams(vector<pair<PatternPointer, int>> &container,
                           const int n) const { //return multiple ngrams, also includes skipgrams!
    const int _n = this->n();
    if (n > _n) return 0;
    int found = 0;
    size_t byteoffset = 0;
    for (int i = 0; i < (_n - n) + 1; i++) {
        container.emplace_back(PatternPointer(*this, 0, n, &byteoffset, true), i);
        found++;
    }
    return found;
}

int PatternPointer::bow_ngrams(std::vector<std::pair<PatternPointer, int>> &container, int n) const {
    const int _n = this->n();
    if (n > _n) return 0;
    int found = 0;
    size_t byteoffset = 0;
    for (int i = 0; i < (_n - n) + 1; i++) {
        container.emplace_back(PatternPointer(*this, 0, n, &byteoffset, true), i);
        container.back().first.compute_bow(n);
        found++;
    }
    return found;
}
...
int
Pattern::subngrams(vector<pair<PatternPointer, int>> &container, int minn, int maxn) const { //also includes skipgrams!
    const int _n = n();
    if (maxn > _n) maxn = _n;
    if (minn > _n) return 0;
    int found = 0;
...
int PatternPointer::ngrams(std::vector<PatternPointer> &container, const int n, const std::vector<Ner> *ners_) const {
    const int _n = this->n();
    if (n > _n) return 0;
    int found = 0;
    size_t byteoffset = 0;
    size_t pattern_start = 0; // token start
    for (int i = 0; i < (_n - n) + 1; i++) {
        ...
        for (; it != ners_->cend() && it->start >= pattern_start && (it->start + it->len) <= (pattern_start + n); it++)
        ...
        if (!pattern_has_ners.empty()) {
            ...
            container.emplace_back(no_ners);
            found++;
        }
    }
    return found;
...
int PatternPointer::ngrams(std::vector<std::pair<PatternPointer, int>> &container, const int n,
                           const std::vector<Ner> *ners_) const {
    const int _n = this->n();
    if (n > _n) return 0;
    int found = 0;
    size_t byteoffset = 0;
...
std::vector<PatternPointer> PatternPointer::bruteforce(const std::vector<Ner> &pattern_has_ners,
                                                       const PatternPointer &no_ner) {//use bit operation to Exhaust all possibilities
    unsigned int mask = 0;
    unsigned int ner_n = pattern_has_ners.size();
    const unsigned int n_bits = ner_n;
...
//        PatternPointer tmp = no_ner.copy();
        ...
        if(mask != 0)
        {
            for (unsigned int i = 0; i < n_bits; i++) {
                if ((mask & (1u << i)) == (1u << i)) {
                    tmp.ners.push_back(pattern_has_ners[pattern_has_ners.size() - 1 - i]);
                }
            }
            reverse(tmp.ners.begin(), tmp.ners.end());  // keep Ner order
        }
        all_commbination.push_back(tmp);
        mask += 1;
    }
    return all_commbination;
}

int PatternPointer::subngrams(std::vector<std::pair<PatternPointer, int>> &container, int minn,
                              int maxn, const std::vector<Ner> *ners_) const {
    const int _n = this->n();
    if (maxn > _n) maxn = _n;
    if (minn > _n) return 0;
    int found = 0;
    for (int i = minn; i <= maxn; i++) {
        found += this->ngrams(container, i, ners_);
    }
    return found;

}

int PatternPointer::subngrams(vector<pair<PatternPointer, int>> &container, int minn,
                              int maxn) const { //also includes skipgrams!
    const int _n = n();
    if (maxn > _n) maxn = _n;
    if (minn > _n) return 0;
    int found = 0;
...
int PatternPointer::bow_subgrams(std::vector<std::pair<PatternPointer, int>> &container, int minn,
                                 int maxn) const {
    const int _n = n();
    if (maxn > _n) maxn = _n;
    if (minn > _n) return 0;
    int found = 0;
    for (int i = minn; i <= maxn; i++) {
        found += this->bow_ngrams(container, i);
    }
    return found;
}
...
unsigned int Pattern::skipcount() const {
    ...
    do {
        ...
        if ((!prevhigh) && (c == ClassDecoder::delimiterclass)) {
            //end marker
            return count;
        } else if (!prevhigh && (((c == ClassDecoder::skipclass) || (c == ClassDecoder::flexclass)) && ((i > 0) ||
                                                                                                        ((data[i - 1] !=
                                                                                                          ClassDecoder::skipclass) &&
                                                                                                         (data[i - 1] !=
                                                                                                          ClassDecoder::flexclass))))) {
            //we have a marker
            count++;
            i++;
        } else {
...
Pattern Pattern::extractskipcontent(const Pattern &instance) const {
    ...
    if (instance.n() != n()) {
        cerr << "WARNING: Extractskipcontent(): instance.n() != skipgram.n(), " << (int) instance.n() << " != "
             << (int) n() << endl;
...
Pattern Pattern::replace(int begin, int length, const Pattern &replacement) const {
    const int _n = n();
    if (begin + length > _n) {
        cerr << "ERROR: Replacing slice " << begin << "," << length << " in a pattern of length " << _n
             << "! Out of bounds!" << endl;
        throw InternalError();
    }
...
Pattern Pattern::addskip(const std::pair<int, int> &gap) const {
    ...
    if (pattern.n() != _n) {
        std::cerr << "ERROR: addskip(): Pattern length changed from " << _n << " to " << pattern.n()
                  << " after substituting slice (" << gap.first << "," << gap.second << ")" << std::endl;
        throw InternalError();
    }
    return pattern;
}
...
Pattern Pattern::addskips(const std::vector<std::pair<int, int> > &gaps) const {
    ...
    for (vector<pair<int, int> >::const_iterator iter = gaps.begin(); iter != gaps.end(); iter++) {
        ...
        if (pattern.n() != _n) {
            std::cerr << "ERROR: addskip(): Pattern length changed from " << _n << " to " << pattern.n()
                      << " after substituting slice (" << iter->first << "," << iter->second << ")" << std::endl;
            throw InternalError();
        }
    }
    return pattern;
...
std::vector<unsigned int> PatternPointer::tovector(unsigned int n) const {
    /*对于patter point 对象，其判断pattern的结束无法根据标志位，只能通过参数n*/
    unsigned int count = 0;
    vector<unsigned int> v;
    if (data == nullptr) return v;
    unsigned int i = 0;
    unsigned int length;
    do {
        const unsigned int cls = bytestoint(data + i, &length);
...
void IndexedCorpus::load(std::istream *in, bool debug) {
    ...
    if (sentence - 1 != sentenceindex.size()) {
        std::cerr << "ERROR: IndexedCorpus expected " << sentence - 1 << " sentence, but loaded only "
                  << sentenceindex.size() << "!! Max size is " << sentenceindex.max_size() << std::endl;
        throw InternalError();
    }
...
PatternPointer
IndexedCorpus::findpattern(const IndexReference &begin, const Pattern &pattern, PatternCategory resultcategory) const {
    if (pattern.category() == NGRAM) {
        ...
        for (std::vector<std::pair<int, int>>::iterator partiter = parts.begin(); partiter != parts.end(); partiter++) {
            ...
            try {
                const PatternPointer candidate = getpattern(
                        IndexReference(begin.sentence, begin.token + partiter->first), partiter->second);
                if (part != candidate) {
                    throw KeyError();
                }
            } catch (KeyError &e) {
            ...
            try {
                ...
                if (candidate == part) {
                    ...
                    if (gapbegin.sentence != 0) {
                        ...
                        newskips.push_back(
                                std::pair<int, int>(gapbegin.token - begin.token, ref.token - gapbegin.token));
                    }
...
void IndexedCorpus::findpattern(std::vector<std::pair<IndexReference, PatternPointer>> &result, const Pattern &pattern,
                                uint32_t sentence, bool instantiate) {
    ...
    for (int i = 0; i < sentencelength - _n; i++) {
        ...
        try {
            PatternPointer p = findpattern(ref, pattern, instantiate ? NGRAM
                                                                     : pattern.category()); //will generate KeyError when not found
            result.push_back(std::pair<IndexReference, PatternPointer>(ref, p));
        } catch (KeyError &e) { ; //ignore and continue
        }
    }
}
...
IndexedCorpus::findpattern(const Pattern pattern, uint32_t sentence, bool instantiate) {
    ...
    IndexReference ref;
    if (sentence == 0) {
        for (std::map<uint32_t, unsigned char *>::iterator iter = sentenceindex.begin();
             iter != sentenceindex.end(); iter++) {
...
std::vector<unsigned int> Pattern::tovector() const {
    vector<unsigned int> v;
    if (data == nullptr) return v;
    unsigned int i = 0;
    unsigned int length;
...
BagOfWords Pattern::get_bag_of_words() const
{
    BagOfWords bagOfWords;
    auto tokens_vec = this->tovector();
    auto tokens = std::set<unsigned int>();
    for(auto cls: tokens_vec)
    {
        if(cls != ClassEncoder::flexclass && cls != ClassEncoder::skipclass && cls != ClassEncoder::delimiterclass)
            tokens.insert(cls);
    }
    bagOfWords.bow.assign(tokens.begin(), tokens.end());

    std::set<string> ners_set;
    for (auto &ner: this->ners) {
        ners_set.insert(ner.raw_string);
    }

    bagOfWords.ners.assign(ners_set.begin(), ners_set.end());
    return bagOfWords;
}

std::vector<unsigned int> Pattern::tovector_with_ner() const {
    ...
    else
    {
        ...
        {
            if(mask[i])
                vec[i] = ClassEncoder::nerclass;
        }
        return vec;
    }
}

void PatternPointer::compute_bow(unsigned int n) {
//    this->bow.clear(); // clear Bag of words information
//
//    auto tokens_vec = this->tovector(n);
//    auto tokens = std::set<unsigned int>(tokens_vec.begin(), tokens_vec.end());
//
//    this->bow.bow.assign(tokens.begin(), tokens.end());
//
//    std::set<string> ners_set;
//    for (auto &ner: this->ners) {
//        ners_set.insert(ner.raw_string);
